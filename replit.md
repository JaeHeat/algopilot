...
- **Dashboard Routing Architecture**: Nested routing system using Wouter Router with base="/dashboard" for all dashboard pages, preventing path duplication issues. Links inside the nested Router use relative paths (e.g., "/creator"), while links outside (sidebar, header) use absolute paths (e.g., "/dashboard/settings"). All dashboard routes properly scoped under /dashboard namespace.
