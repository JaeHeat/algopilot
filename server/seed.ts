import { db } from "./db";
import { users, bots, botPerformance, botTradeLogs, botPerformanceHistory, subscriptions, exchangeConnections, creatorPosts } from "@shared/schema";
...
async function seed() {
  ...
  if (createdBots.length > 0) {
    const samplePosts = [
      {
        ...
        creatorId: createdBots[0].creatorId,
        content: "Exciting market conditions today! Our momentum indicators are showing strong signals in BTC and ETH. The bot has executed 3 profitable trades this morning with an average return of 2.1%. Stay tuned for more updates.",
        postType: "text",
      ...
      },
      {
        botId: createdBots[0].id,
        creatorId: createdBots[0].creatorId,
        content: "Strategy Update: I've optimized the RSI threshold parameters based on recent market volatility. The bot will now be slightly more conservative in sideways markets to protect capital. Expect fewer trades but higher win rate.",
        postType: "text",
        isAutoGenerated: false,
      ...
      },
      {
        botId: createdBots[0].id,
        creatorId: createdBots[0].creatorId,
        content: "Weekend recap: +8.3% returns this week! The momentum strategy performed exceptionally well during the volatility spike on Thursday. All positions closed profitably. Looking forward to next week's opportunities.",
        postType: "text",
        isAutoGenerated: false,
      ...
      },
      {
        ...
        creatorId: createdBots[1].creatorId,
        content: "Grid adjustments made: I've tightened the grid spacing by 10% to better capture the current ranging behavior in BTC/USDT. This should increase trading frequency while maintaining our risk profile.",
        postType: "text",
        isAutoGenerated: false,
      ...
      },
      {
        botId: createdBots[1].id,
        creatorId: createdBots[1].creatorId,
        content: "Performance milestone reached! Our Grid Trader Pro just completed its 1,000th profitable trade since launch. Average profit per trade: 0.8%. Consistency is key in grid trading.",
        postType: "text",
        isAutoGenerated: false,
      ...
      },
      {
        ...
        creatorId: createdBots[2].creatorId,
        content: "High volatility alert: Market conditions are perfect for scalping today. The bot has already executed 47 trades with an impressive 89% win rate. Quick in, quick out - that's how we do it!",
        postType: "text",
        isAutoGenerated: false,
      ...
      },
      {
        ...
        creatorId: createdBots[3].creatorId,
        content: "DCA Update: Consistent accumulation continues. This week we added to positions across BTC, ETH, and SOL. Remember, DCA is a marathon, not a sprint. Patience and discipline will be rewarded.",
        postType: "text",
        isAutoGenerated: false,
    ...
    await db.insert(creatorPosts).values(samplePosts).onConflictDoNothing();
    console.log(`Created ${samplePosts.length} creator posts`);
  }
